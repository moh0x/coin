...
const userControler = require('../../controller/transformation/transformController');
...
router.patch('/verifyAccount',body("verifyCode").isString().isLength({min:1,max:6}),verifyToken,userControler.confirmAccountFunc);
router.get('/userInfo',verifyToken,verifyUser,userControler.getUserInfo);
router.patch('/resetPassword',body("resetPasswordCode").isString().isLength({min:1,max:6}),body('password').isString().isLength({min:8,max:30}),userControler.resetPasswordFunc);//email
...
router.post('/sendResetCode',userControler.sendResetCodeFunc);
