...
router.patch('/verifyAccount',body("verifyCode").isString().isLength({min:1,max:5}),verifyToken,userControler.confirmAccountFunc);
router.get('/userInfo',verifyToken,verifyUser,userControler.getUserInfo);
router.patch('/resetPassword',body("email").isEmail().isLength({min:6,max:50}).withMessage("type valid email"),body("resetPasswordCode").isString().isLength({min:1,max:5}),body("resetPasswordCode").isString().isLength({min:1,max:6}),body('password').isString().isLength({min:8,max:30}),userControler.resetPasswordFunc);//email
